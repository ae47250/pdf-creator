...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ consoleLogin?: string }> }) {
  ...
  return (
    <main className="page-shell">
      ...
      {enabled && !signedIn && <section className="panel login-panel"><div className="login-copy"><h2>Console sign in</h2><p>Enter the shared console password to open the internal testing controls.</p>{loginFailed && <p className="login-error" role="alert">Password not recognized.</p>}</div><form className="login-form" action="/api/console/session" method="post"><label htmlFor="console-password">Password<input id="console-password" name="password" type="password" autoComplete="current-password" required /></label><button type="submit">Open test console</button></form></section>}
